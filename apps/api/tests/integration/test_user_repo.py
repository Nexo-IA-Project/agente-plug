...
async def test_count_admins(db_session: AsyncSession) -> None:
    repo = UserRepository(db_session)
    await repo.save(
        User(account_id=2, name="A", email="a@x.com", password_hash="h", role=UserRole.ADMIN)
    )
    await repo.save(
        User(account_id=2, name="B", email="b@x.com", password_hash="h", role=UserRole.OPERATOR)
    )
    await repo.save(
        User(account_id=2, name="C", email="c@x.com", password_hash="h", role=UserRole.ADMIN)
    )
    await db_session.commit()

    count = await repo.count_active_admins(account_id=2)
    assert count == 2
