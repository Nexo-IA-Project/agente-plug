...
async def test_guard_returns_ignorado_when_loja_already_delivered():
    ...
    uc = EnviarFollowup(
        repo=repo, chatnexo=chatnexo, scheduler=scheduler, loja_express_port=loja_express_port
    )
    result = await uc.execute(
        account_id=1, contact_id="5511999990000", conversation_id="conv-1", day=1
    )
...
async def test_d3_sends_check_template_and_updates_status():
    ...
    result = await uc.execute(
        account_id=1, contact_id="5511999990000", conversation_id="conv-1", day=3
    )
    chatnexo.send_template.assert_called_once()
    call_kwargs = chatnexo.send_template.call_args.kwargs
...
async def test_d5_transfers_to_human_when_not_delivered():
    ...
    result = await uc.execute(
        account_id=1, contact_id="5511999990000", conversation_id="conv-1", day=5
    )
...
async def test_d7_sends_template_and_transfers_to_human():
    ...
    result = await uc.execute(
        account_id=1, contact_id="5511999990000", conversation_id="conv-1", day=7
    )
    chatnexo.send_template.assert_called_once()
    call_kwargs = chatnexo.send_template.call_args.kwargs
