...
async def test_embed_returns_list_of_floats():
    ...
    mock_client.embeddings.create = AsyncMock(return_value=MagicMock(data=[embedding_data]))
...
async def test_embed_batch_returns_list_of_embeddings():
    ...
    mock_client.embeddings.create = AsyncMock(return_value=MagicMock(data=[e1, e2]))
